...
class GrowthRecord
{
    ...
    {
        $stmt = $this->pdo->prepare('SELECT * FROM growth_records WHERE batch_id = :batch_id ORDER BY date DESC');
    ...
    {
        $stmt = $this->pdo->prepare('SELECT * FROM growth_records WHERE record_id = :record_id');
    ...
    {
        $stmt = $this->pdo->prepare('DELETE FROM growth_records WHERE record_id = :record_id');
        return $stmt->execute(['record_id' => $recordId]);
    }

    public function latestAverageWeight(int $batchId): ?float
    {
        $stmt = $this->pdo->prepare('SELECT average_weight_kg FROM growth_records WHERE batch_id = :batch_id ORDER BY date DESC LIMIT 1');
        $stmt->execute(['batch_id' => $batchId]);
